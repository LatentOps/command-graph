...
from .trajectory_corpus import AgentTrajectory, TrajectoryResult, run_agent_trajectory_corpus
...
class RegressionReplay:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "RegressionReplay":
        ...
        if kind == "trajectory" and any(
            (
                semantics_raw is not None,
                policy_raw is not None,
                bool(observations_raw),
                bool(expected_trajectory_categories),
                bool(expected_provenance_codes),
            )
        ):
            raise ValueError(f"{replay_id} trajectory replays do not support safety-only overrides")
...
class RegressionReplayResult:
    ...
    def critical_false_allow(self) -> bool:
        if self.replay.severity != "critical":
            return False
        if self.trajectory_result is not None:
            return any(
                step.actual == "allow" and step.expected != "allow"
